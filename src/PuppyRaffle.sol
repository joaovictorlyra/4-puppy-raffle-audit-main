...
// @audit-info: use of floating pragma is not the ideal
...
contract PuppyRaffle is ERC721, Ownable {
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        ...
        // are we emitting an event if the array is empty?
        // @follow-audit 
    ...
    function getActivePlayerIndex(address player) external view returns (uint256) {
        // @audit-issue -> unbounded loop can lead to DoS. Remove the loops and use a mapping instead. But in this case the DoS should've already happened in the enterRaffle. But it's still unefficent usage of gas
    ...
    function selectWinner() external {
        ...
        // @audit-issue -> not really random
        uint256 winnerIndex =
            uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, block.difficulty))) % players.length;
        address winner = players[winnerIndex];
        // Q: why not use address(this).balance?
